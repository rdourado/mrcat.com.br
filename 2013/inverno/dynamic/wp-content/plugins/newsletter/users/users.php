...
class NewsletterUsers extends NewsletterModule {
    ...
    function export() {
        ...
        while (true) {
            ...
            $wpdb->update( "{$wpdb->prefix}newsletter", array( 'list_1' => 1 ), array( 'list_1' => 0 ) );
            for ($i = 0; $i < count($recipients); $i++) {
                $convmap = array( 0x0100, 0xFFFF, 0, 0xFFFF );
                $encutf = mb_encode_numericentity( $recipients[$i]->name, $convmap, 'UTF-8' );
                $recipients[$i]->name = utf8_decode( $encutf );

                echo '"' . $recipients[$i]->email . 
                '";"' . $recipients[$i]->sex .
                '";"' . $this->sanitize_csv($recipients[$i]->name) . '";';
