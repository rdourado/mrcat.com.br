...
function wdgt_tw($args = '') {
	...
	if ('yes' == $showtags) {
		$tags = tags_get_tags(array_merge($args, array('minnum' => $minnum, 'maxnum' => $maxnum, 'orderby' => 'count', 'order' => 'DESC'))); // Always query top tags
	}
	...
	if (is_wp_error($return))
		...
		echo apply_filters('wdgt_tw', $return, $args);
	}
}
...
function tw($args = '') {
	...
	$options = get_option('template_tw');
	...
	if (is_wp_error($return))
		...
		echo apply_filters('tw', $return, $args);
	}
}

// generate_tag_cloud() - function to create the links for the cloud based on the args from the tw() function
...
function generate_tag_cloud($tagscats, $args = '') {
	...
	foreach ($counts as $tag => $count) {
		...
		if ($format=='list') {
			$a[] = "<li><a href=\"$tag_link\" title=\"".attribute_escape(sprintf(__('%d topics'), $count))."\"$rel 
							style=\"font-size: ".$tag_weight."$unit;".(isset($tag_color) ? " color: $tag_color;" : "")."\">$tag</a>"
					.('yes' == $showcount ? " $postcount" : "")."</li>";
		} elseif ($format=='drop') {
			$a[] = "<option value='$tag_link'>$tag".('yes' == $showcount ? " $postcount" : "")."</option>";
		} else {
			$a[] = "<a href=\"$tag_link\" class=\"tw-tag tag-link-$tag_id\" title=\"".attribute_escape(sprintf(__('%d topics'), $count))."\"$rel style=\"font-size: ".$tag_weight
	...
	case 'array' :
		...
		break;
	case 'list' :
		$return = "<ul class='tw-tag-cloud'>\n\t";
		$return .= join("\n\t", $a);
		$return .= "\n</ul>\n";
		break;
	case 'drop' :
		$return = "\n<select name=\"tw-dropdown\" onchange='document.location.href=this.options[this.selectedIndex].value;'>\n\t<option value=\"\">Select Tag</option>\n\t";
		$return .= join("\n\t", $a);
		$return .= "</option>\n</select>\n";
		break;
	default :
...
function install_defs() {
	...
	add_option('template_tw');
	add_option('widget_tw');

	update_option('widget_tw',$defaults);
	update_option('template_tw',$defaults);
}
